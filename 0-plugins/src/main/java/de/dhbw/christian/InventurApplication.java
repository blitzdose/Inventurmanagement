...
import de.dhbw.christian.plugins.persistence.hibernate.InventoryItem.HibernateInventoryItemRepository;
...
public class InventurApplication {
    ...
    private static Object dependencyInjection(Class<?> type, EntityManager entityManager) throws InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        Arrays.stream(type.getConstructors()[0].getParameterTypes()).forEach(aClass -> {
            ...
            } else if (aClass == InventoryItemApplicationService.class) {
                parameterObjects.add(new InventoryItemApplicationService(new HibernateInventoryItemRepository(entityManager), new InventoryItemDomainService()));
            }
        });
